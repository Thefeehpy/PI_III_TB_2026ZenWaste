  ...
  useEffect(() => {
    ...
    fetchAddressByCep(normalizedCep)
      ...
      .catch((error) => {
        ...
        setCepLookupMessage(error instanceof Error ? error.message : "Não foi possível consultar o CEP.");
      });
  ...
  const attachPhoto = async (file: File) => {
    ...
    try {
      ...
      toast({
        ...
        description: "A imagem foi adicionada ao anúncio e será publicada junto com ele.",
      });
    } catch (error) {
      toast({
  ...
  const handleSuggestDescription = async () => {
    if (!form.title || !form.type) {
      toast({
        ...
        description: "Selecione um item e informe o título antes de gerar a descrição.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: response.source === "ai" ? "Descrição gerada por IA" : "Descrição sugerida",
        description:
          response.source === "ai"
            ? "Revise o texto antes de publicar."
            : response.message || "IA indisponível; usei uma sugestão padrão.",
      });
    } catch (error) {
      toast({
        title: "Descrição não gerada",
  ...
  const handleSuggestPrice = async () => {
    if (!form.type) {
      toast({
        ...
        description: "Selecione um item antes de sugerir o preço.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      setPriceInsight(response.insight);
      toast({
        title: response.source === "ai" ? "Preço sugerido por IA" : "Preço sugerido",
        description:
          response.source === "ai"
            ? "Revise o valor antes de publicar."
            : response.message || "IA indisponível; usei histórico de mercado.",
      });
    } catch (error) {
      toast({
        title: "Preço não sugerido",
        description: error instanceof Error ? error.message : "Não foi possível sugerir o preço.",
        variant: "destructive",
      });
    } finally {
      setIsSuggestingPrice(false);
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <Card>
        <CardContent className="p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                  ...
                  {form.photos[0] ? (
                    <div className="space-y-4">
                      <div className="overflow-hidden rounded-xl border border-border/70 bg-background">
                        <img src={form.photos[0]} alt="Prévia da imagem do anúncio" className="h-64 w-full object-cover" />
                      </div>

                      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div className="min-w-0">
                          <p className="truncate text-sm font-medium text-foreground">
                            {selectedPhotoName || "Imagem pronta para o anúncio"}
                          </p>
                    ...
                    <div className="flex flex-col items-center justify-center px-4 py-8 text-center">
                      ...
                      <p className="mt-4 text-sm font-medium text-foreground">
                        Clique aqui ou arraste uma imagem para anexar ao anúncio
                      </p>
                      <p className="mt-2 max-w-md text-sm text-muted-foreground">
                        Aceita JPG, PNG, WEBP e outros formatos de imagem. A imagem anexada será usada como foto do
                        anúncio publicado.
                      </p>
                    </div>
                  )}
                ...
                <p className="text-xs text-muted-foreground">
                  O upload é salvo no anúncio publicado. Se preferir, você ainda pode publicar sem imagem e usar a
                  imagem padrão do marketplace.
                </p>
              </div>
            </div>
          )}
          ...
          {currentStep === 3 && (
            <div className="space-y-4">
              <CardTitle className="text-lg">Revisão do Anuncio</CardTitle>

              {form.photos[0] && (
                <div className="overflow-hidden rounded-2xl border border-border/70 bg-background">
                  <img src={form.photos[0]} alt="Prévia final da imagem do anúncio" className="h-64 w-full object-cover" />
                </div>
              )}
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex flex-col gap-1 border-b border-border py-2 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <span className="font-medium">{selectedPhotoName || "Imagem padrão do marketplace"}</span>
                </div>
                {form.description && (
                  <div className="py-2">
