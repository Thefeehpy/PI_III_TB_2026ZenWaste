  ...
  return (
    <div className="space-y-6">
      <Card className="relative overflow-hidden rounded-[34px] border-border/60 bg-[linear-gradient(135deg,#091423_0%,#10344d_38%,#12735f_100%)] text-white shadow-[0_28px_80px_rgba(15,23,42,0.24)]">
        ...
        <CardContent className="relative grid gap-8 p-6 md:p-8 xl:grid-cols-[1.1fr_0.9fr] xl:items-end">
          <div className="space-y-5">
            ...
            <div>
              <h2 className="text-3xl font-semibold tracking-tight sm:text-4xl">Visao Geral</h2>
              <p className="mt-3 max-w-2xl text-sm leading-7 text-white/78 sm:text-base">
                Uma leitura mais estrategica dos produtos cadastrados, dos tipos de residuos presentes no estoque e da
                atividade recente da operacao.
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-3 text-sm text-white/78">
              <span className="rounded-full border border-white/12 bg-white/8 px-3 py-1.5">
                {itemsWithBalance} produtos com saldo disponivel
              </span>
              <span className="rounded-full border border-white/12 bg-white/8 px-3 py-1.5">
                {uniqueTypes} tipos catalogados
              </span>
              <span className="rounded-full border border-white/12 bg-white/8 px-3 py-1.5">
                {weeklyMovements} movimentacoes nos ultimos 7 dias
              </span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          },
          {
            ...
            subtitle: "entradas e saidas em 7 dias",
            icon: Activity,
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
        <Card className="rounded-[32px] border-border/70 shadow-[0_14px_36px_rgba(15,23,42,0.05)]">
          <CardHeader>
            ...
            <CardDescription>Veja quais tipos de residuos concentram mais produtos cadastrados.</CardDescription>
          </CardHeader>
          <CardContent>
            {typeBarData.length === 0 ? (
              <div className="flex h-[320px] items-center justify-center text-center text-sm text-muted-foreground">
                Cadastre produtos no estoque para visualizar a distribuicao por tipo.
              </div>
            ) : (
        ...
        <Card className="rounded-[32px] border-border/70 shadow-[0_14px_36px_rgba(15,23,42,0.05)]">
          <CardHeader>
            ...
            <CardDescription>Distribuicao visual dos principais tipos presentes no estoque.</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          ...
          <CardContent className="space-y-4">
            {topProducts.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-border bg-muted/20 p-6 text-center text-sm text-muted-foreground">
                Os produtos com maior saldo aparecerao aqui assim que houver itens cadastrados.
              </div>
            ) : (
        ...
        <Card className="rounded-[32px] border-border/70 shadow-[0_14px_36px_rgba(15,23,42,0.05)]">
          <CardHeader>
            <CardTitle className="text-xl">Ultimas atualizacoes</CardTitle>
            <CardDescription>Produtos atualizados mais recentemente para leitura rapida da operacao.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {recentItems.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-border bg-muted/20 p-6 text-center text-sm text-muted-foreground">
                As atualizacoes mais recentes aparecem aqui quando o estoque receber novos produtos.
              </div>
            ) : (
